...
bool tabu_list[6][6][6] = {};                       //tabu list
...
{
    int value = (7 - k); // for k=1..7 value=7..1
    ...
    for (int iter = 0; iter < 2; ++iter)
    {
        for (int i = 0; i < 6; ++i)
        {
            ...
            {
            case 0:
                forward_clue = CLUES[23 - i] - k;
                backward_clue = CLUES[i + 6] - k;
                break;
            case 1:
                forward_clue = CLUES[i] - k;
                backward_clue = CLUES[17 - i] - k;
                break;
            } //switch

            if (forward_clue > 0)
                for (int j = 0; j < forward_clue; ++j)
                    if (target_matrix[i][j] > 6 || target_matrix[i][j] < 1)
                        target_matrix[i][j] = 9;

            if (backward_clue > 0)
                for (int j = 5; j > (5 - backward_clue); --j)
                    if (target_matrix[i][j] > 6 || target_matrix[i][j] < 1)
                        target_matrix[i][j] = 9;
                    ...
                    for (int jk = 0; jk < 6; ++jk)
                        if (target_matrix[ik][jk] == value)
                        {
                            for (int jj = 0; jj < 6; ++jj)
                                if (target_matrix[ik][jj] > 6 || target_matrix[ik][jj] < 1)
                                    target_matrix[ik][jj] = 9; //row

                            for (int ii = 0; ii < 6; ++ii)
                                if (target_matrix[ii][jk] > 6 || target_matrix[ii][jk] < 1)
...
pair<int, int> GetClues(vector<int> &state)
{
    ...
    for (register int i = 0; i < 6; ++i)
    {
        ...
        else if (!state[i] && MaxFloorF != 6)
        {
            ++forward;
        }
        if (state[6 - (i + 1)] > MaxFloorB)
        {
            MaxFloorB = state[6 - (i + 1)];
            ++backward;
        }
        else if (!state[6 - (i + 1)] && MaxFloorB != 6)
        {
            ++backward;
        }
    }
...
bool ValidState(int row, int col)
{
    ...
    for (register int iter = 0; iter < 2; ++iter)
    {
        ...
        {
            i = row;
            forward_clue = CLUES[23 - i];
            backward_clue = CLUES[i + 6];
        }
        else
        {
            i = col;
            forward_clue = CLUES[i];
            backward_clue = CLUES[17 - i];
            TransposeMatrix(target_matrix);
        }
...
{
    vector<int> one_six = {1, 2, 3, 4, 5, 6};

    do
    {
        auto clues = GetClues(one_six);
        permutations.insert(make_pair(clues, one_six));
    } while (next_permutation(one_six.begin(), one_six.end()));
}
...
void UpdateTabuList()
{
    ...
    for (int row = 0; row < 6; ++row)
    {
        for (int iter = 0; iter < 2; ++iter)
        {
            ...
            {
            case 0:
                forward_clue = CLUES[23 - row];
                backward_clue = CLUES[6 + row];

                break;
            case 1:
                forward_clue = CLUES[row];
                backward_clue = CLUES[17 - row];
...
void FillDLXSPM()
{
    ...
    int column_shift = 6 * 6, row_count = 1;
    ...
    for (int i = 0; i < 6; ++i)
    {
        for (int j = 0; j < 6; ++j)
        {
            for (int k = 0; k < 6; ++k)
            {
                second_column_row = (k + 6 * i) + column_shift;
                third_column_col = (k + 6 * j) + column_shift * 2;
...
std::vector<std::vector<int>> SolvePuzzle(const std::vector<int> &clues)
{
    ...
    if (first_lunch)
    {
        ...
        DLXMatrix.reserve(217);
        covered_nodes.reserve(36);
        covered_nodes_dcp.reserve(36 * 3);
        sub_covered_nodes.reserve(36 * 3);
        CLUES.reserve(24);
    ...
    for (int i = 0; i < 6; ++i)
        for (int j = 0; j < 6; ++j)
            for (int k = 0; k < 6; ++k)
                if (tabu_list[i][j][k])
                {
                    int first_column_cell = i * 6 + j;
                    int second_column_row = (k + 6 * i) + 36;
                    int third_column_col = (k + 6 * j) + 36 * 2;
                    int row_count = i * 36 + j * 6 + (k + 1);
    ...
    for (int i = 0; i < 6; ++i)
        for (int j = 0; j < 6; ++j)
            if (target_matrix[i][j])
            {
                ...
                //row and column
                int first_column_cell = i * 6 + j;
                int second_column_row = ((target_matrix[i][j] - 1) + 6 * i) + 36;
                int third_column_col = ((target_matrix[i][j] - 1) + 6 * j) + 36 * 2;
                int row_count = i * 36 + j * 6 + target_matrix[i][j];
